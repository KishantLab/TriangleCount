...
__global__
void Calculate_indegree(unsigned long long *d_col_idx, unsigned long long *in_degrees, unsigned long long num_edges) {
    unsigned long long tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < num_edges) {
        atomicAdd(&in_degrees[d_col_idx[tid]], 1);
        //printf("%llu", in_degrees[d_col_idx[tid]]);
    }
}
                               ...
                               unsigned long long num_edges)
{
    ...
    double total_cleaning_time = 0.0;
		// unsigned long long *d_row_ptr_G;
		// unsigned long long *d_col_idx_G;
    ...
    for (unsigned long long value = 0; value < nopart; ++value)
    {
        ...
        unsigned long long *row_ptr_final = (unsigned long long *)malloc((h_combined_nodes_s+1) * sizeof(unsigned long long));
        unsigned long long *col_idx_final = (unsigned long long *)malloc(ci_pos * sizeof(unsigned long long));
        cudaMemcpy(row_ptr_final, d_row_ptr_dir, (h_combined_nodes_s+1) * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
        cudaMemcpy(col_idx_final, d_values_out, (temp_arr_sum_cpu[h_combined_nodes_s-1]) * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
        unsigned long long Final_Nodes = h_combined_nodes_s;
        unsigned long long Final_Edges = temp_arr_sum_cpu[h_combined_nodes_s-1];
        ...
        printf("h_combined_nodes_s: %llu, indices_size: %llu\n",h_combined_nodes_s, indices_size);
        ...
        for(int z = 0; z < 5; z++)
        {
            ...
            printf("For loop Done new combined nodes found!!!\n");
            cudaFree(d_indegree);
            cudaFree(d_h_combined_nodes);

            if(h_combined_nodes_s == new_combined_node_s){
              cudaFree(d_values_out);
              cudaFree(d_row_ptr_dir);
              break;
            }

            printf("Round %d Cleanning started\n",z);
            ...
            cout<<endl;
            cudaMemcpy(row_ptr_final, d_row_ptr_clean, (new_combined_node_s+1) * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
            cudaMemcpy(col_idx_final, d_col_idx_clean, (temp_arr_sum1[new_combined_node_s-1]) * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
            // free(indices);
            // free(h_halo_nodes);
            // free(filtered_halo_nodes);
            // free(h_combined_nodes);
            ...
            unsigned long long *d_indegree, *d_values_out, *d_row_ptr_dir;
            cudaMallocManaged(&d_indegree, num_nodes * sizeof(unsigned long long));
            cudaMemset(d_indegree, 0, num_nodes*sizeof(unsigned long long));

            grid_size_G = (num_nodes + BLOCK_SIZE) / BLOCK_SIZE;
            Calculate_indegree<<<grid_size_G, BLOCK_SIZE>>>(d_col_idx_clean, d_indegree, num_nodes);
            cudaDeviceSynchronize();

            cudaMallocManaged(&d_h_combined_nodes, new_combined_node_s*sizeof(unsigned long long));
            cudaMemcpy(d_h_combined_nodes, d_new_combined_nodes, new_combined_node_s * sizeof(unsigned long long), cudaMemcpyDeviceToHost);

            printf("New Indegree: ");
            for(int i=0; i<num_nodes; i++)
            {
              printf("%llu ",d_indegree[i]);
            }
            printf("\nNew combined nodes: ");
            for( int i=0; i<new_combined_node_s; i++)
            {
              printf("%llu ",d_h_combined_nodes[i]);
            }
            printf("\n");

                      // Calculate_indegree(unsigned long long *d_col_idx_clean, unsigned long long *in_degrees, unsigned long long num_edges)
            cudaMallocManaged(&d_row_ptr_dir, (new_combined_node_s+1) * sizeof(unsigned long long));
            cudaMallocManaged(&d_values_out, (temp_arr_sum1[new_combined_node_s-1]) * sizeof(unsigned long long));

            cudaMemcpy(d_row_ptr_dir, d_row_ptr_clean, (new_combined_node_s+1) * sizeof(unsigned long long), cudaMemcpyHostToDevice);
            cudaMemcpy(d_values_out, d_col_idx_clean, (temp_arr_sum1[new_combined_node_s-1]) * sizeof(unsigned long long), cudaMemcpyHostToDevice);
            Final_Nodes = new_combined_node_s;
            Final_Edges = temp_arr_sum1[new_combined_node_s-1];
            ...
            printf("File Writting Started!!!!\n");
            outfile <<Final_Nodes <<" "<<Final_Nodes+1 <<" " << Final_Edges <<" "<< indices_size<< endl;
            // cout<<"Row Pointer: "<<endl;
            for(int i = 0; i<Final_Nodes+1; i++){
                outfile<<row_ptr_final[i]<<" ";
                // cout<<row_ptr_dir[i]<<" ";
            }
            outfile<<endl;
            cout<<endl;
            ...
            for(int i = 0; i<Final_Edges; i++){
                outfile<<col_idx_final[i]<<" ";
                // cout<<col_idx_dir[i]<<" ";
            }
            outfile<<endl;
            cout<<endl;
            printf("File Writting Done!!!!!\n");
    ...
    cout<<"Total cleaning Time: "<<total_cleaning_time<<" seconds."<<endl;
