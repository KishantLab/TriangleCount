...
__global__
void Convert(unsigned long long int *d_in_deg, unsigned long long int *d_h_combined_nodes, unsigned long long int *d_row_ptr, unsigned long long int *d_col_idx,
            unsigned long long int *temp_arr_sum, unsigned long long int *d_row_ptr_Dir, unsigned long long int *d_col_idx_Dir,
                               ...
                               unsigned long long num_edges)
{
    ...
    for (unsigned long long value = 0; value < nopart; ++value)
    {
        ...
        for(int i = 0; i<1; i++){
            unsigned long long *d_new_combined_nodes, *d_new_halo_nodes;
            ...
            unsigned long long pos = indices_size;
            for(int j = indices_size; j<h_combined_nodes_s; j++){
                if(d_indegree[d_h_combined_nodes[j]]!=0){
                    d_new_combined_nodes[pos] = d_h_combined_nodes[j];
                    d_new_halo_nodes[pos2] = d_h_combined_nodes[j];
                    pos++;
                    pos2++;
                }
            }
            // cout<<"New Halo Nodes: "<<endl;
            // for(int i = 0; i<pos2; i++){
            //     cout<<d_new_halo_nodes[i]<<" ";
            // }
            // cout<<endl;
            // cout<<"New Combined Nodes: "<<endl;
            // for(int i = 0; i<pos; i++){
            ...
            // cudaFree(d_indegree);
            cudaFree(d_h_combined_nodes);
            grid_size_G = (pos + BLOCK_SIZE) / BLOCK_SIZE;
            // grid_size_G = pos + BLOCK_SIZE / BLOCK_SIZE;
            cudaMallocManaged(&temp_arr, pos * sizeof(unsigned long long));
            // Find_size<<<grid_size_G, BLOCK_SIZE>>>(d_indegree, d_new_combined_nodes, d_row_ptr_dir, d_col_idx_dir, temp_arr, h_combined_nodes_s, pos,
            // h_combined_nodes_s+1, d_row_ptr_dir[h_combined_nodes_s], d_new_halo_nodes, pos2, d_induced_nodes, indices_size);

            Find_size<<<grid_size_G, BLOCK_SIZE>>>(d_indegree, d_new_combined_nodes, d_row_ptr_G, d_col_idx_G, temp_arr, num_nodes, pos,
            num_nodes+1, num_edges, d_new_halo_nodes, pos2, d_induced_nodes, indices_size);

            cudaDeviceSynchronize();

            h_combined_nodes_s = pos;
            void *d_temp_storage4 = NULL;
            cudaMallocManaged(&temp_arr_sum, pos * sizeof(unsigned long long));

            cub::DeviceScan::InclusiveSum(d_temp_storage4, temp_storage_bytes, temp_arr, temp_arr_sum, pos);

            // Allocate temporary storage for inclusive prefix sum
            cudaMallocManaged(&d_temp_storage4, temp_storage_bytes);

            // Run inclusive prefix sum
            cub::DeviceScan::InclusiveSum(d_temp_storage4, temp_storage_bytes, temp_arr, temp_arr_sum, pos);

            // unsigned long long *temp_arr_sum_cpu = (unsigned long long *)malloc(pos * sizeof(unsigned long long));

            unsigned long long *d_temp_indegree;
            cudaMallocManaged(&d_temp_indegree, num_nodes*sizeof(unsigned long long));
            cudaMemcpy(d_temp_indegree, d_indegree, num_nodes * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
            cudaFree(temp_arr);
            cudaFree(d_temp_storage4);
            // cudaFree(d_indegree);

            unsigned long long *d_row_ptr_dir2, *d_col_idx_dir2;
            cudaMallocManaged(&d_row_ptr_dir2, (pos+1) * sizeof(unsigned long long));
            cudaMallocManaged(&d_col_idx_dir2, (temp_arr_sum[pos-1]) * sizeof(unsigned long long));
            // cudaMallocManaged(&d_indegree, num_nodes*sizeof(unsigned long long));
            cudaMemset(d_indegree, 0, num_nodes*sizeof(unsigned long long));
            ...
            for(unsigned long long i = 0; i<pos; i++){
                d_index_array[d_new_combined_nodes[i]] = i;
            }

      //       Convert<<<grid_size_G, BLOCK_SIZE>>>(d_temp_indegree, d_new_combined_nodes, d_row_ptr_dir, d_col_idx_dir, temp_arr_sum, d_row_ptr_dir2, d_col_idx_dir2,
      //       h_combined_nodes_s, pos, h_combined_nodes_s+1, d_row_ptr_dir[h_combined_nodes_s], d_index_array, d_new_halo_nodes, pos2,
      //       d_induced_nodes, indices_size, d_indegree);
      //       cudaDeviceSynchronize();

            Convert<<<grid_size_G, BLOCK_SIZE>>>(d_temp_indegree, d_new_combined_nodes, d_row_ptr_G, d_col_idx_G, temp_arr_sum, d_row_ptr_dir2, d_col_idx_dir2,
            num_nodes, pos, pos, num_edges, d_index_array, d_new_halo_nodes, pos2, d_induced_nodes, indices_size, d_indegree);
            cudaDeviceSynchronize();

            unsigned long long int ci_pos = d_row_ptr_dir[pos];
            unsigned long long int rp_pos = (pos+1);
            ...
            cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_col_idx_dir2, d_values_out,
                ci_pos, rp_pos-1, d_row_ptr_dir2, d_row_ptr_dir2 + 1);
            // Allocate temporary storage
            ...
            checkCuda(cudaFree(d_col_idx_dir2));
            checkCuda(cudaFree(d_temp_storage));

            unsigned long long *row_ptr_dir = (unsigned long long *)malloc((pos+1) * sizeof(unsigned long long));
            unsigned long long *col_idx_dir = (unsigned long long *)malloc(d_row_ptr_dir[pos] * sizeof(unsigned long long));
            cudaMemcpy(row_ptr_dir, d_row_ptr_dir2, (pos+1) * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
            cudaMemcpy(col_idx_dir, d_values_out, d_row_ptr_dir[pos] * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
