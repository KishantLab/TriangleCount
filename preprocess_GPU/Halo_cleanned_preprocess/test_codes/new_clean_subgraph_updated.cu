...
__device__ __forceinline__ unsigned long long int BSearch (unsigned long long int skey , unsigned long long int *neb, unsigned long long int start, unsigned long long int sizelist)
{
	unsigned long long int total = 0;
	if(skey < neb[start] || skey > neb[sizelist])
	{
		return 0;
	}
	else if(skey == neb[start] || skey == neb[sizelist])
	{
		return 1;
	}
	else
...
__global__ void Find_size(unsigned long long int *d_in_deg, unsigned long long int *d_h_combined_nodes, unsigned long long int *d_row_ptr,
                            ...
                            unsigned long long filtered_halo_nodes_size,
    ...
    if(id < h_combined_nodes_s)
    {
      ...
      for(unsigned long long int j=d_row_ptr[src]; j<d_row_ptr[src+1]; j++)
      {
        ...
        if(deg_src < deg_dst)
        {
          ...
          unsigned long long flag1 = BSearch(dest, d_h_combined_nodes, indices_size, h_combined_nodes_s-1);
          unsigned long long flag2 = BSearch(dest, d_h_combined_nodes, 0, indices_size-1);
          if(flag1==1 || flag2==1){
            count++;
          }
...
void Convert(unsigned long long int *d_in_deg, unsigned long long int *d_h_combined_nodes, unsigned long long int *d_row_ptr, unsigned long long int *d_col_idx, unsigned long long int *temp_arr_sum, unsigned long long int *d_row_ptr_Dir, unsigned long long int *d_col_idx_Dir,
            ...
            unsigned long long *d_index_array, unsigned long long filtered_halo_nodes_size,
            unsigned long long indices_size, unsigned long long *d_indegree)
{
    unsigned long long int id = threadIdx.x + blockIdx.x * blockDim.x ; //Define id with thread id
...
__global__ void thrust_sorting_col_idx(unsigned long long *row_ptr, unsigned long long *col_idx, unsigned long long rp_size) {
    unsigned long long id = threadIdx.x + blockDim.x * blockIdx.x;
    if(id < rp_size) {
        unsigned long long start = row_ptr[id];
        unsigned long long end = row_ptr[id+1];

        // Create device pointers for the segment to be sorted
        thrust::device_ptr<unsigned long long> begin(col_idx + start);
        thrust::device_ptr<unsigned long long> end_ptr(col_idx + end);

        // Sort the segment using Thrust
        thrust::sort(thrust::device, begin, end_ptr);
    }
}
                               ...
                               unsigned long long num_edges)
{
    ...
    for (unsigned long long value = 0; value < nopart; ++value)
    {
        ...
        num_nodes+1, num_edges,  unique_halo_nodes_s[0],  indices_size);
        cudaDeviceSynchronize();
        ...
        num_nodes, h_combined_nodes_s, h_combined_nodes_s, num_edges, d_index_array,  unique_halo_nodes_s[0],
        indices_size, d_indegree);
        cudaDeviceSynchronize();
