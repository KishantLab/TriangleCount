...
for value in range(nopart):
    ...
    print(indices)
    # index_array.extend(indices.tolist())
    # for index, elem in enumerate(d_node_parts):
        # if elem == value:
    ...
    # induced_subgraph = dgl.node_subgraph(g, induced_nodes)
    # end_halo = time.time()
    # print("induced subgraph created!!!\t time of construction is : ",round((end_halo - start_halo),4), "seconds")

    # Find halo nodes
    # start_halo = time.time()
    # halo_nodes = find_halo_nodes(g, induced_nodes)

        # Use PyTorch for GPU operations
    # csr_matrix = g.adj_tensors('csr')
    # induced_nodes_mask = torch.zeros(g.num_nodes(), dtype=torch.bool, device=induced_nodes.device)
    # induced_nodes_mask[induced_nodes] = True

    # successors = csr_matrix @ induced_nodes_mask
    # predecessors = csr_matrix.T @ induced_nodes_mask

    # Combine successors and predecessors
    # all_neighbors = successors.nonzero(as_tuple=False).flatten().unique()
    # all_neighbors = torch.cat([all_neighbors, predecessors.nonzero(as_tuple=False).flatten().unique()]).unique()

    # Remove induced nodes from the halo nodes
    # halo_nodes = all_neighbors[~induced_nodes_mask[all_neighbors]]
    # induced_nodes_list = induced_nodes.tolist()
    # successors = g.successors(induced_nodes_list).to(device).unique()
    # predecessors = g.predecessors(induced_nodes_list).to(device).unique()
    # all_neighbors = torch.cat([successors, predecessors]).unique()
    # halo_nodes = all_neighbors[~torch.isin(all_neighbors, induced_nodes)]
    # end_halo = time.time()
    # successors = g.successors(induced_nodes).unique()
    # predecessors = g.predecessors(induced_nodes).unique()
    # all_neighbors = torch.cat([successors, predecessors]).unique()
    # all_neighbors = torch.cat([g.successors(induced_nodes).flatten(), g.predecessors(induced_nodes).flatten()]).unique()
    # halo_nodes = all_neighbors[~torch.isin(all_neighbors, induced_nodes)]
    # end_halo = time.time()
    ...
    # org_id = np.array(induced_with_halo_subgraph.ndata['orig_id'])
    # org_id_s = len(org_id)
