                                ...
                                unsigned long long *d_induced_nodes, unsigned long long *d_pos, unsigned long long num_edges)
{
    ...
    if (bid < induced_nodes_s)
    {
        ...
        for (unsigned long long i = threadIdx.x; i < deg; i+= BLOCK_SIZE)
        {
            ...
            unsigned long long dest = d_col_idx_G[start+i];
            unsigned long long flag = Search(dest, d_induced_nodes, induced_nodes_s-1);
...
__global__ void find_halo_nodes2(unsigned long long *d_row_ptr_G, unsigned long long *d_col_idx_G,
                                signed long long *d_halo_nodes, unsigned long long induced_nodes_s,
                                unsigned long long *d_induced_nodes, unsigned long long *d_pos, unsigned long long num_edges)
{
    unsigned long long bid = blockIdx.x * blockDim.x + threadIdx.x;
    if (bid < induced_nodes_s)
    {
        unsigned long long node = d_induced_nodes[bid];
        unsigned long long start = d_row_ptr_G[node];
        ...
        {
            // unsigned long long flag = 0;
            unsigned long long dest = d_col_idx_G[i];
            unsigned long long flag = Search(dest, d_induced_nodes, induced_nodes_s-1);
            if (flag == 0)
            {
                      // unsigned long long pos = d_pos[0];
                    ...
                    //     printf("Bid: %llu, Tid: %llu, d_pos: %llu \n", bid, i, d_pos[0]);
                    // }
                    if(count<num_edges && d_halo_nodes[count]==-1){
                        d_halo_nodes[count] = dest;
												count++;
												// d_pos[0] = d_pos[0] +1;
                    }
            }
...
__global__ void sorting_col_idx(unsigned long long *row_ptr, unsigned long long *col_idx, unsigned long long rp_size) {
	/* code */
	...
	if(id < rp_size)
	{
		...
		for(unsigned long long i = start; i< end; i++)
		{
			for(unsigned long long j= i+1; j<end; j++)
			{
				...
				{
          unsigned long long temp = col_idx[i];
          col_idx[i] = col_idx[j];
          col_idx[j] = temp;
                               ...
                               unsigned long long num_edges)
{
    ...
    for (unsigned long long value = 0; value < nopart; ++value)
    {
        ...
        // Finding indices of the partitio
				struct timeval begin_exe, end_exe;
				gettimeofday(&begin_exe, 0);
        ...
        for(int z = 0; z<5; z++)
        {
            ...
            // cudaFree(d_h_combined_nodes);
            // cudaFree(d_indegree);
						cout<<"h_combined_nodes_s: "<<h_combined_nodes_s<<endl;
						cout<<"new_combined_node_s: "<<new_combined_node_s<<endl;
            ...
            clean_Find_size<<<grid_size_G, BLOCK_SIZE>>>(temp_arr1, d_row_ptr_dir, d_col_idx_dir, d_new_combined_nodes, new_combined_node_s);
            cudaDeviceSynchronize();
            ...
            // }
            // cout<<endl;
						cudaMemset(d_row_ptr_dir, 0, (h_combined_nodes_s+1)*sizeof(unsigned long long));
						cudaMemset(d_col_idx_dir, 0, Num_Edges*sizeof(unsigned long long));
						Num_Nodes = new_combined_node_s;
						Num_Edges = temp_arr_sum1[new_combined_node_s-1];
						cudaMemcpy(d_row_ptr_dir, d_row_ptr_clean, (new_combined_node_s+1) * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
						cudaMemcpy(d_col_idx_dir, d_col_idx_clean, (temp_arr_sum1[new_combined_node_s-1]) * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
						cudaMemset(d_h_combined_nodes, 0, h_combined_nodes_s*sizeof(unsigned long long));
						h_combined_nodes_s = new_combined_node_s;
						cudaMemset(d_indegree, 0, num_nodes*sizeof(unsigned long long));
						cudaMemset(d_row_ptr_clean, 0, (new_combined_node_s+1)*sizeof(unsigned long long));
						cudaMemset(d_col_idx_clean, 0, (temp_arr_sum1[new_combined_node_s-1])*sizeof(unsigned long long));

						grid_size_G = (Num_Edges + BLOCK_SIZE) / BLOCK_SIZE;
						Calculate_indegree<<<grid_size_G, BLOCK_SIZE>>>(d_col_idx_clean, d_indegree, Num_Edges);
						cudaDeviceSynchronize();
						cudaMemcpy(d_h_combined_nodes, d_new_combined_nodes, new_combined_node_s * sizeof(unsigned long long), cudaMemcpyDeviceToHost);

						// printf("Calculate_indegree done ");
						// cout<<"Indegree: "<<endl;
		        // for(int i = 0; i<num_nodes; i++){
		        //     cout<<d_indegree[i]<<" ";
		        // }
		        // cout<<endl;
						...
						double elapsed_exe = seconds_exe + microseconds_exe*1e-6;
						cout<<"Time measured of round: "<<elapsed_exe<<" seconds."<<endl;
						cout << "Kernel code completed" << endl;
						total_exe_time += elapsed_exe;

            // // Update number of nodes and edges in the partition
            outfile <<Num_Nodes <<" "<<Num_Nodes+1 <<" " << Num_Edges <<" "<< indices_size<< endl;
            // cout<<"Row Pointer: "<<endl;
            for(int i = 0; i<Num_Nodes+1; i++){
                outfile<<d_row_ptr_dir[i]<<" ";
                // cout<<row_ptr_dir[i]<<" ";
            }
            outfile<<endl;
            cout<<endl;
            ...
            for(int i = 0; i<Num_Edges; i++){
                outfile<<d_col_idx_dir[i]<<" ";
								// outfile<<d_clean_col_idx[i]<<" ";
                // cout<<col_idx_dir[i]<<" ";
            }
            outfile<<endl;
            cout<<endl;
