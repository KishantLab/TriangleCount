...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		//Writing to file
		fprintf(wfile, "%llu ", Vertex);
		fprintf(wfile, "%llu", Edges);
		fprintf(wfile, "\n");
			...
			//	for (unsigned long long int i = 0; i < 1; i++)
		{
			...
			fprintf(wfile, "%llu ", v_pos);
			//fscanf(file, "%llu", &t_ver);
			fprintf(wfile, "%llu ", rp_pos);
			fprintf(wfile, "%llu ", ci_pos);
			fprintf(wfile, "%llu", t_ver);
			fprintf(wfile, "\n");
			...
			for (unsigned long long int j=0; j<rp_pos; j++)
			{
				fprintf(wfile, "%llu ", new_row_ptr[j]);
				//new_row_ptr[j]=data;
			...
			cudaMemcpy(new_col_index,d_values_out,sizeof(unsigned long long int)*ci_pos,cudaMemcpyDeviceToHost);
			printf("sorting done\n");
			scanf("%d", &x);
			for (unsigned long long int j=0; j<ci_pos; j++)
			{
				fprintf(wfile, "%llu ", new_col_index[j]);
				//new_col_index[j]=data;
			}
			fprintf(wfile, "\n");
			printf("writing done\n");
			...
			printf("press key for kernel calling\n");
			scanf("%d", &x);
			...
			checkCuda(cudaGetLastError());
			printf("kernel execute successfully\n");
			scanf("%d", &x);
			...
			printf("summesion execute successfully\n");
			scanf("%d", &x);
