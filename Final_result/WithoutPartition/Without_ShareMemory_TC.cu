...
__global__ void Find_Triangle(unsigned long long int *g_col_index, unsigned long long int *g_row_ptr ,unsigned long long int *g_sum )
{
  ...
  unsigned long long int start;
	...
	//unsigned long long int triangle = 0;
		...
		{
			unsigned long long int start2 = g_row_ptr[g_col_index[i+start]];
			unsigned long long int end2 = g_row_ptr[g_col_index[i+start]+1];
			unsigned long long int size_list2 = end2 - start2;
			unsigned long long int M = ceil((float)(size_list2)/N_THREADS_PER_BLOCK);
			for( unsigned long long int k = 0; k < M; k++)
			{
				...
				if(id < size_list2)
				{
					...
					result = Search(g_col_index[id+start2],g_col_index,end-1,start);
					//printf("\nedge(%llu , %llu) : %llu , tid : %llu, size_list1 :%llu , size_list2: %llu, start2 :%llu , end2 :%llu skey:%llu, g_col_index[0]:%llu ,g_col_index[%llu]:%llu , id :%llu, M:%llu",bid, g_col_index[i], result,tid,size_list1+1,size_list2+1,start2,end2,g_col_index[id+start2],g_col_index[start],size_list1,g_col_index[end], id, M);
