...
__global__ void Find_Triangle(int *d_col_index, int *d_row_ptr, int *d_vertex_arr,unsigned long long int *d_sum, int ci_pos, int rp_pos)
{
...
int main(int argc, char *argv[])
{
	...
	else
	{
			...
			//for (int i = 0; i < 1; i++)
		{
			...
			Find_Triangle<<<t_ver,N_THREADS_PER_BLOCK>>>(d_col_index,d_row_ptr,d_vertex_arr,d_sum,ci_pos,rp_pos);
			cudaDeviceSynchronize();
