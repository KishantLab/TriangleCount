...
/*	
	...
	for (int x = list1_start; x < list1_end; x++)
	{
		...
		while( d_col_index[end-1] < d_col_index[x] && start < list2_end)
		{
			start = end;
			end = end + step;
			if( end > list2_end)
	...
	*/
	for (int x = list1_start; x < list1_end; x++)
	{
		...
		int lo = start, hi = end, mi;
		while( hi - lo > 1)
		{
			mi = (hi + lo)/2;
			if( d_col_index[mi] < d_col_index[x]){lo = mi-1;}
			else if( d_col_index[mi] > d_col_index[x]){hi = mi;}
			else if( d_col_index[mi] == d_col_index[x])
			{
				total++;
