...
__global__ void Find_Triangle(unsigned long long int *d_col_index, unsigned long long int *d_row_ptr,unsigned long long int *d_sum, unsigned long long int ci_pos, unsigned long long int rp_pos,unsigned long long int v_pos)
{
...
int main(int argc, char *argv[])
{
	...
	else
	{
      ...
      //fscanf(file, "%llu", &t_ver);
			...
			checkCuda(cudaMalloc(&d_sum,sizeof(unsigned long long int)*1));
			//float total_kernel_time = 0.0 ;
			...
			Find_Triangle<<<t_ver,N_THREADS_PER_BLOCK>>>(d_col_index,d_row_ptr,d_sum,ci_pos,rp_pos,t_ver);
			checkCuda(cudaDeviceSynchronize());
