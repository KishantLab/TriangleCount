		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			for( int k = 0; k < M; k++)
			{
				...
				if(id <= size_list2) 
				{
					...
					//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d skey:%d, neb[0]:%d ,neb[%d]:%d",bid, neb[i], result,tid,size_list1,size_list2,start2,end2,g_col_index[id+start2],neb[0],size_list1,neb[size_list1]);
