...
__global__ void Find_Triangle(int *g_col_index, int *g_row_ptr, int vertex, int edge ,int *g_sum )
{
	...
	int triangle = 0;
	if(size_list1 < SHARED_MEM && size_list1 < N_THREADS_PER_BLOCK)
	{
		if(tid <= size_list1)
		{
		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			for( int k = 0; k < M; k++)
			{
				...
				if(id <= size_list2) 
				{
					...
					//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d skey:%d, neb[0]:%d ,neb[%d]:%d Fit",bid, neb[i], result,tid,size_list1+1,size_list2+1,start2,end2,g_col_index[id+start2],neb[0],size_list1,neb[size_list1]);
	...
	{
		int N = ceil((float)(size_list1 +1)/ SHARED_MEM);
		int remining_size = size_list1;
		int size = SHARED_MEM-1;
		for( int i = 0; i < N; i++)
		{
			int id = SHARED_MEM * i + tid;
			if( remining_size > size)
			{
				if(tid <= size && id <= size_list1)
				{
				...
				for( int j = start; j <= end; j++)
				{
					...
					for( int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, tempid :%d, skey :%d, N:%d, I:%d, remining_size:%d, size:%d, neb[0]:%d, neb[%d]:%d if ",bid, g_col_index[j], result,tid,size_list1+1,size_list2+1,start2,end2,tempid,g_col_index[tempid+start2],N,i,remining_size,size,neb[0],size,neb[size]);
							//atomicAdd(&g_sum[0],result);
							//printf("\nedge(%d , %d) src : %d dst :%d ", bid,g_col_index[j],size_list1+1,size_list2+1);
							triangle += result;
						}
						//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d, remining_size:%d, size:%d, neb[0]:%d, neb[%d]:%d M:%d if ",bid, g_col_index[j], triangle,tid,size_list1,size_list2,start2,end2,id,g_col_index[tempid+start2],N,i,remining_size,size,neb[0],size,neb[size],M);
			...
			else
			{
				...
				if(tid <= remining_size && id <= size_list1)
				{
					neb[tid] = g_col_index[id+start];
					//printf(" neb : %d", neb[tid]);
				}
