		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			for( int k = 0; k < M; k++)
			{
				...
				if(id <= size_list2) 
				{
					...
					printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d skey:%d",bid, neb[i], result,tid,size_list1,size_list2,start2,end2,g_col_index[id+start2]);
				...
				for( int j = start; j <= end; j++)
				{
					...
					for( int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d, remining_size:%d, size:%d, if ",bid, g_col_index[j], result,tid,size_list1,size_list2,start2,end2,id,g_col_index[tempid+start2],N,i,remining_size,size);
							atomicAdd(&g_sum[0],result);
						}
					}
					__syncthreads();
				...
				for( int j = start; j <= end; j++)
				{
					...
					for (int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d else",bid, g_col_index[j], result,tid,size_list1,size_list2,start2,end2,id,g_col_index[tempid+start2],N,i);
							atomicAdd(&g_sum[0],result);
