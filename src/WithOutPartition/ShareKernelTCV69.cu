...
int main(int argc, char *argv[])
{
	...
	checkCuda( cudaMemcpy(g_col_index,col_index,sizeof(int)*vertex,cudaMemcpyHostToDevice) );
	checkCuda( cudaMemcpy(g_row_ptr,row_ptr,sizeof(int)*edge,cudaMemcpyHostToDevice) );

	printf("\nData copied Successfull");
	//int x;
	//scanf("%d", &x);
	...
	printf("\nSearch : %.6f sec Vertex : %d Edge : %d Triangle : %llu",milliseconds/1000,vertex,edge*2,Triangle);
