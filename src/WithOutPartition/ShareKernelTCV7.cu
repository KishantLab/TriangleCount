...
__device__ int Range(int skey, int *neb, int sizelist, int id)
{
	if(id ==0 && skey > neb[sizelist])
	{
		return 0;
	}
	if(id == sizelist && skey < neb[0]) 
	{
		return 0;	
	}
	//if(tid == skey < neb[0] || skey > neb[sizelist])
	//{
	//	return 1;
	//}
	return 1;
	...
	if(size_list1 < N_THREADS_PER_BLOCK)
	{
		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			for( int k = 0; k < M; k++)
			{
				...
				if(id <= size_list2) 
				{
					if( id==0 || id==size_list2)
					{
						range = Range(g_col_index[id+start2],neb,size_list1,id);
						atomicAdd(&temp,range);
					}
				}
				__syncthreads();
				if(id <= size_list2)
				{
					if(temp >= 1)
					{
						...
						//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d skey:%d, neb[0]:%d ,neb[%d]:%d, temp:%d",bid, neb[i], result,tid,size_list1,size_list2,start2,end2,g_col_index[id+start2],neb[0],size_list1,neb[size_list1],temp);
					}
					triangle += result;
				...
				for( int j = start; j <= end; j++)
				{
					...
					for( int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							if( tid==0 || tid==size)
							{
								range = Range(g_col_index[tempid+start2],neb,size,tid);
								atomicAdd(&temp,range);
							}
						}
						//__syncthreads();
						if(tempid <= size_list2)
						{
							if(temp >= 1)
							{
								...
								//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d, remining_size:%d, size:%d, neb[0]:%d, neb[%d]:%d if ",bid, g_col_index[j], result,tid,size_list1,size_list2,start2,end2,id,g_col_index[tempid+start2],N,i,remining_size,size,neb[0],size,neb[size]);
							}
							triangle += result;
							//atomicAdd(&g_sum[0],result);
						}
				...
				for( int j = start; j <= end; j++)
				{
					...
					for (int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							if(tempid <= size_list2) 
							{
								if( tid==0 || tid==remining_size)
								{
									range = Range(g_col_index[tempid+start2],neb,remining_size,tid);
									atomicAdd(&temp,range);
								}
							}
							//__syncthreads();
							if(tempid <= size_list2)
							{
								if(temp >= 1)
								{
									...
									//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d neb[0]:%d, neb[%d]:%d, else",bid, g_col_index[j], result,tid,size_list1,size_list2,start2,end2,id,g_col_index[tempid+start2],N,i,neb[0],remining_size,neb[remining_size]);
								}
