...
__global__ void Find_Triangle(int *g_col_index, int *g_row_ptr, int vertex, int edge ,int *g_sum )
{
	...
	if(size_list1 < N_THREADS_PER_BLOCK)
	{
		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			int end2 = g_row_ptr[neb[i]+1];
			int size_list2 = end2 - start2;
			if(tid <= size_list2) 
			{
				int result = 0;
				result = Search(g_col_index[tid+start2],neb,size_list1);
				printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d skey:%d",bid, neb[i], result,tid,size_list1,size_list2,start2,end2,g_col_index[tid+start2]);
	...
	else
	{
		...
		int remining_size = size_list1 +1 ;
		int size = N_THREADS_PER_BLOCK;
		for( int i = 0; i < N; i++)
		{
			...
			if( remining_size > size)
			{
				...
				for( int j = start; j <= end; j++)
				{
					...
					{
						int result = 0;
						result = Search(g_col_index[tid+start2],neb,size);
						printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d if",bid, g_col_index[j], result,tid,size_list1,size_list2,start2,end2,id,g_col_index[tid+start2],N,i);
						atomicAdd(&g_sum[0],result);
					}
					//__syncthreads();
				}
				remining_size = remining_size - size;
			}
			else
			{
				...
				for( int j = start; j <= end; j++)
				{
					...
					{
						int result = 0;
						result = Search(g_col_index[tid+start2],neb,remining_size);
						printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d else",bid, g_col_index[j], result,tid,size_list1,size_list2,start2,end2,id,g_col_index[tid+start2],N,i);
						atomicAdd(&g_sum[0],result);
