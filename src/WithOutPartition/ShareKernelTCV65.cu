...
__device__ int Search (int skey , int *neb, int sizelist, int flag,int tid, int sizelist2, int lo, int hi)
{
	int total = 0;
	if(tid == 0)
	{
		if(skey > neb[sizelist]){flag=1;}
	}
	if(tid == sizelist2)
	{
		if(skey < neb[0]){flag=1;}
	}
		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			for( int k = 0; k < M; k++)
			{
				...
				if(id <= size_list2) 
				{
					...
					int mid = size_list1/2;
					int lo = 0;
					int hi = size_list1;
					if ( skey < neb[mid])
					{
						hi = mid;
						result = Search(skey,neb,size_list1,flag,tid,size_list2,lo,hi);
					}
					else
					{
						lo = mid;
				...
				for( int j = start; j <= end; j++)
				{
					...
					for( int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							//result = Search(g_col_index[tempid+start2],neb,size,flag,tid,size_list2);
				...
				for( int j = start; j <= end; j++)
				{
					...
					for (int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							//result = Search(g_col_index[tempid+start2],neb,remining_size,flag,tid,size_list2);
