...
__device__ int Range(int skey, int *neb, int sizelist, int tid, int sizelist2, int flag)
{
	int temp = 0;
	int hi = sizelist;
	int lo = 0;
	int mid = (hi+lo)/2;
	if(tid ==0 && skey > neb[sizelist])
	{
		flag = 1;
	}
	if(tid == sizelist2 && skey < neb[0])
	{
		flag = 1;
	}
	if(flag == 1){return 0;}
	else if( skey < neb[mid])
	{
		hi = mid;
		temp = Search(skey,neb,sizelist,lo,hi);
		return temp;
	}
	else 
	{
		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			for( int k = 0; k < M; k++)
			{
				...
				if(id <= size_list2) 
				{
					...
					result = Range(g_col_index[id+start2],neb,size_list1,tid,size_list2,flag);
				...
				for( int j = start; j <= end; j++)
				{
					...
					for( int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							result = Range(g_col_index[tempid+start2],neb,size,tid,size_list2,flag);
				...
				for( int j = start; j <= end; j++)
				{
					...
					for (int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							result = Range(g_col_index[tempid+start2],neb,remining_size,tid,size_list2,flag);
