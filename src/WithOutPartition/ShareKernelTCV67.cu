...
__device__ int Search (int skey , int *neb, int sizelist, int lo, int hi)
{
	int total = 0;
	//return 0;
	if(skey < neb[lo] || skey > neb[hi])
	{
		return 0;
	}
	else if(skey == neb[lo] || skey == neb[hi])
	{
		return 1;
	}
	else
...
__global__ void Find_Triangle(int *g_col_index, int *g_row_ptr, int vertex, int edge ,int *g_sum )
{
	...
	int triangle = 0;
	int step = (int)floor(sqrtf(size_list1));
		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			for( int k = 0; k < M; k++)
			{
				...
				if(id <= size_list2) 
				{
					...
					int hi = size_list1;
					//int mid = (lo+hi)/2;
					int skey = g_col_index[id+start2];
					/*
					while(neb[hi] < skey && lo < size_list1)
					{
						lo = hi;
						hi = hi+step*2;
						if(hi > size_list1-1){hi = size_list1;}
					}
					*/
					result = Search(skey,neb,size_list1,lo,hi);
					/*
					   if( skey < neb[mid])
					   {
					   hi = mid;
				...
				for( int j = start; j <= end; j++)
				{
					...
					for( int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							int hi = step*2;
							//int mid = (lo+hi)/2;
							int skey = g_col_index[tempid+start2];
							while(neb[hi] < skey && lo < size)
							{
								lo = hi;
								hi = hi+step*2;
								if(hi > size_list1-1){hi = size;}
							}
							result = Search(skey,neb,size,lo,hi);
							/*
							   if( skey < neb[mid])
							   {
							   hi = mid;
				...
				for( int j = start; j <= end; j++)
				{
					...
					for (int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							int result = 0;
							int lo = 0;
							int hi = remining_size;
							//int mid = (lo+hi)/2;
							int skey = g_col_index[tempid+start2];
							/*while(neb[hi] < skey && lo < remining_size)
							{
								lo = hi;
								hi = hi+step*2;
								if(hi > size_list1-1){hi = remining_size;}
							}
							*/
							result = Search(skey,neb,remining_size,lo,hi);
							/*	
								if( skey < neb[mid])
								{
								hi = mid;
