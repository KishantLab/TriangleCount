...
__global__ void Find_Triangle(int *g_col_index, int *g_row_ptr, int vertex, int edge ,int *g_sum, int *Reordered_Array)
{
	...
	//if(tid ==0)
	//{
		start = g_row_ptr[index];
		end = g_row_ptr[index+1]-1;
	//}
	__syncthreads();
		...
		for( int i = 0; i <= size_list1; i++)
		{
			...
			for( int k = 0; k < M; k++)
			{
				...
				if(id <= size_list2) 
				{
					...
					//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d skey:%d, neb[0]:%d ,neb[%d]:%d",index, neb[i], result,tid,size_list1,size_list2,start2,end2,g_col_index[id+start2],neb[0],size_list1,neb[size_list1]);
					//atomicAdd(&g_sum[0],result);
					//printf("\nedge(%d , %d) src : %d dst :%d ", index,neb[i],size_list1,size_list2);
					triangle += result;
				}
			}
			__syncthreads();
				...
				for( int j = start; j <= end; j++)
				{
					...
					for( int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d, remining_size:%d, size:%d, neb[0]:%d, neb[%d]:%d if ",index, g_col_index[j], result,tid,size_list1,size_list2,start2,end2,id,g_col_index[tempid+start2],N,i,remining_size,size,neb[0],size,neb[size]);
							//atomicAdd(&g_sum[0],result);
							//printf("\nedge(%d , %d) src : %d dst :%d ", index,g_col_index[j],size_list1,size_list2);
							triangle += result;
						}
					}
					__syncthreads();
				...
				for( int j = start; j <= end; j++)
				{
					...
					for (int k = 0; k < M; k++)
					{
						...
						if(tempid <= size_list2) 
						{
							...
							//printf("\nedge(%d , %d) : %d , tid : %d, size_list1 :%d , size_list2: %d, start2 :%d , end2 :%d, id :%d, skey :%d, N:%d, I:%d neb[0]:%d, neb[%d]:%d, else",index, g_col_index[j], result,tid,size_list1,size_list2,start2,end2,id,g_col_index[tempid+start2],N,i,neb[0],remining_size,neb[remining_size]);
							//atomicAdd(&g_sum[0],result);
							//printf("\nedge(%d , %d) src : %d dst :%d ", index,g_col_index[j],size_list1,size_list2);
							triangle += result;
						}
...
int main(int argc, char *argv[])
{
	...
	int *reorderd_array;
	reorderd_array = (int *)malloc(sizeof(int)*NUM_VERTICES);
	int *Reorderd_Array;
	cudaMalloc(&Reorderd_Array,sizeof(int)*NUM_VERTICES);
	...
	if(file1 == NULL)
	{
		printf("Partition File Not Opended\n");
		exit(0);
	}
	...
	fclose(file1);
	//printf("Data Array :");
	//for(int i=0; i<=vertex; i++){printf("%d ",data_array[i]);}
	int pos=0;
	...
	*/
	...
	cudaMemcpy(Reorderd_Array,reorderd_array,sizeof(int)*NUM_VERTICES,cudaMemcpyHostToDevice);
	//****************KERNEL CALLED *****************
	cudaEventRecord(start3);
	Find_Triangle<<<vertex,N_THREADS_PER_BLOCK>>>(g_col_index,g_row_ptr,vertex,edge,g_sum,Reorderd_Array);
