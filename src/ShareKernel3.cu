...
#define SHARED_MEM 512



//---------------------Binary Search -------------------------//
__device__ unsigned long long int Search (unsigned long long int skey , unsigned long long int *neb, unsigned long long int sizelist)
{
...
__global__ void Find_Triangle(unsigned long long int *d_col_index, unsigned long long int *d_row_ptr, unsigned long long int *d_vertex_arr,unsigned long long int *d_sum, unsigned long long int ci_pos, unsigned long long int rp_pos)
{
...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		//	for (unsigned long long int i = 0; i < 1; i++)
		{
			...
			unsigned long long int v_pos=0, rp_pos=0, ci_pos=0, t_ver=0;
			...
			cudaMalloc(&d_vertex_arr,sizeof(unsigned long long int)*v_pos);

			unsigned long long int *d_row_ptr;   // GPU MEMORY ALLOCATION
			cudaMalloc(&d_row_ptr,sizeof(unsigned long long int)*rp_pos);

			unsigned long long int *d_col_index;  //GPU MEMORY ALOOCATION
			cudaMalloc(&d_col_index,sizeof(unsigned long long int)*ci_pos);
			...
			cudaMalloc(&d_sum,sizeof(unsigned long long int)*1);
			//float total_kernel_time = 0.0 ;
			...
			cudaMemcpy(d_col_index,new_col_index,sizeof(unsigned long long int)*ci_pos,cudaMemcpyHostToDevice);
			cudaMemcpy(d_row_ptr,new_row_ptr,sizeof(unsigned long long int)*rp_pos,cudaMemcpyHostToDevice);
			cudaMemcpy(d_vertex_arr,vertex_arr,sizeof(unsigned long long int)*v_pos,cudaMemcpyHostToDevice);
			...
			cudaMalloc(&d_values_in,sizeof(unsigned long long int)*ci_pos);

			unsigned long long int  *d_values_out;      // e.g., [-, -, -, -, -, -, -]
			cudaMalloc(&d_values_out,sizeof(unsigned long long int)*ci_pos);
			...
			Find_Triangle<<<v_pos,N_THREADS_PER_BLOCK>>>(d_col_index,d_row_ptr,d_vertex_arr,d_sum,ci_pos,rp_pos);
			cudaDeviceSynchronize();
